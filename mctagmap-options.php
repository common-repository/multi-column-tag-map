...
<?php
	...
	if(isset($_POST["submit"])){ 
		if ( !isset($_POST['mctm98mqo']) || !wp_verify_nonce($_POST['mctm98mqo'], plugin_basename( __FILE__ )) ){
	...
	if(is_array($mctmarr)){
		if(!isset($mctmarr['use_custom'])){
			$mctmarr['use_custom'] = '';
		}
		if(!isset($mctmarr['use_conditional'])){
			$mctmarr['use_conditional'] = '';
		}
	} else {
		$mctmarr = array();
		$mctmarr['use_custom'] = '';
		$mctmarr['use_conditional'] = '';
		$mctmarr['mctm_css_custom'] = '';
		$mctmarr['mctm_css_conditional'] = '';
	}
...
<textarea name="mctm_css_conditional"><?php if($mctmarr['mctm_css_conditional']){ echo esc_html($mctmarr['mctm_css_conditional']); } else { echo ''; } ?></textarea>
			...
			<?php echo wp_nonce_field( plugin_basename( __FILE__ ), 'mctm98mqo', true, false); ?>			
		</form>
